...
class Cell(object):
    ...
    spaninfo: list
        if cell is the top-left cell in the span cells area,
        it' spaninfo = [spanrow, spancol, rowspan, colspan],
        else it' spaninfo = []
        spanrow is the row index of the right-bottom cell in the span cells area,
        spancol is the col index of the right-bottom cell in the span cells area,
        rowspan is the number of span cell in the same span cells area horizontally,
        colspan is the number of span cell in the same span cells area vertically.
    text : string
...
class Table(object):
    ...
    def __init__(self, cols, rows):
        ...
        self.shape = (0, 0)
        self.accuracy = 0
        #TODO: improve error_text's value and output
        self.error_text = []
        self.whitespace = 0
        self.order = None
        self.page = None
    ...
    def set_span(self):
        ...
        """
        row_num, col_num = len(self.rows), len(self.cols)
            ...
            for j in range(col_num):
                ...
                if cell.bound == 4:
                    ...
                    if not left and (right and top and bottom):
                        ...
                        cell.hspan = True
                        for c in range(j+1, col_num):
                            if self.cells[i][c].right:
                                cell.spaninfo = [i, c, 1, c-j+1]
                                break
                    elif not top and (left and right and bottom):
                        cell.vspan = True
                    elif not bottom and (left and right and top):
                        cell.vspan = True
                        for r in range(i+1, row_num):
                            if self.cells[r][j].bottom:
                                cell.spaninfo = [r, j, r-i+1, 1]
                                break
                elif cell.bound == 2:
    ...
    def to_excel(self, path, **kwargs):
        ...
        worksheet = workbook.add_sheet('page-{}-table-{}'.format(self.page, self.order))
        cn, rn = self.shape
        for i in range(cn):
            for j in range(rn):
                if self.cells[i][j].spaninfo:
                    value = self.cells[i][j].text.replace("\n", "")
                    spanrow = self.cells[i][j].spaninfo[0]
                    spancol = self.cells[i][j].spaninfo[1]

                    try:
                        worksheet.write_merge(i, spanrow, j, spancol, value)
                    except:
                        # TODO IMPROE
                        print(sys.exc_info())
                        self.error_text.append("[{}][{}][{}][{}]{}".format(i, j, spanrow, spancol, value))
        worksheet.write_merge(i + 3, i + 3, 0, 0, self.error_text)
        workbook.save(path)
        ...
        if f in ["csv", "json", "html"]:
            ...
            for table in self._tables:
                ...
                for i in range(cn):
                    row_list = []
                    for j in range(rn):
                        if table.cells[i][j].spaninfo:
                            value = table.cells[i][j].text.replace("\n", "")
                            rowspan = table.cells[i][j].spaninfo[2]
                            colspan = table.cells[i][j].spaninfo[3]
                            row_list.append({"value": value, "rowspan": rowspan, "colspan": colspan})
                    records.append(row_list)
                with open(filepath, 'w', encoding='utf-8') as f:
                    f.write(str({"title": "page:{}-order:{}".format(table.page, table.order), "records": records}))
        elif f == "excel":
            filepath = os.path.join(dirname, basename[:-1])
            # Angelia changed it to output merge cells
            workbook = xlwt.Workbook()
            for table in self._tables:
                worksheet = workbook.add_sheet('page-{}-table-{}'.format(table.page, table.order))
                cn, rn = table.shape
                for i in range(cn):
                    for j in range(rn):
                        if table.cells[i][j].spaninfo:
                            value = table.cells[i][j].text.replace("\n", "")
                            spanrow = table.cells[i][j].spaninfo[0]
                            spancol = table.cells[i][j].spaninfo[1]
                            try:
                                worksheet.write_merge(i, spanrow, j, spancol, value)
                            except:
                                #TODO IMPROE
                                print(sys.exc_info())
                                table.error_text.append("[{}][{}][{}][{}]{}".format(i,j,spanrow,spancol,value))
                worksheet.write_merge(i + 3, i + 3, 0, 0, table.error_text)
                workbook.save(os.path.join(filepath + ".xls"))
            # writer.save()
            if compress:
                zipname = os.path.join(os.path.dirname(path), root) + ".zip"
                with zipfile.ZipFile(zipname, "w", allowZip64=True) as z:
                    z.write(filepath + ".xls", os.path.basename(filepath))
                    # print(filepath + ".xls")
